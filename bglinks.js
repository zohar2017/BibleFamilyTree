window.BGLinks = (function () {
  ...
  var updateURLs = function () {
    bgHost = "https://www.biblegateway.com";
    toolsHost = "https://www.biblegateway.com/share/tooltips/data";
    cdHost = "https://www.biblegateway.com/public/link-to-us/tooltips";
  };

  var insertBiblerefs = function (node) {
    var new_nodes;
    if (node.nodeType === 3) {
      // don't need update string
  ...
  var searchNode = function (node, inserted_nodes) {
    ...
    if (verse_match == null) {
      ...
      while ((matched = book_chap_regex.exec(verse_match[0]))) {
        ...
        verse = matched[4];
        // from here update modify
        //node.parentNode.setAttribute("data-ref", "Gen 1:1");
        //var newLinkNode = document.createElement("a");
        //newLinkNode.className = "bibleref";
        //newLinkNode.target = "_BLANK";
        var passage = book + " " + chapter + ":" + verse;
        //bibleref updated in dndtree-apend-text
        //node.parentNode.setAttribute("bibleref", passage);
        //newLinkNode.href =
        //  bgHost +
        //  "/passage/?search=" +
        //  passage +
  ...
  // update a element to text element
  var addBiblerefListeners = function () {
    var links = document.getElementsByTagName("text");
    for (var i = 0; i < links.length; i++) {
      var link = links[i];

      if ((link.getAttribute("bibleref") && link.getAttribute("bibleref") != "") || (link.getAttribute("scriptRef") && link.getAttribute("scriptRef") != "")) {
        if (that.clickTooltip !== true) {
  ...
  var toggleTooltip = function (e) {
    ...
    //e is the TEXT object, will be popup to show
    var reference;
    var bibleref;
    if ((bibleref = link.getAttribute("bibleref"))) {
      reference = bibleref;
    } else {
      if (link.getAttribute("scriptRef") && link.getAttribute("scriptRef") != "") {
        reference = "ref" + e.target.innerHTML;
      } else {
  ...
  var showTooltip = function (e, link) {
    ...
    //update first tooltop when scriptRef not ""
    ...
    if (tooltip === null) {
      ...
      //find if the bible tag include scriptRef
      if (e.target.getAttribute("scriptRef") && e.target.getAttribute("scriptRef") != "" && $(tooltip).find("div[name=\"scriptRef\"]").length === 0) {
        that.scriptRef = "<div class=\"bg_popup-more\" name=\"scriptRef\">" + e.target.getAttribute("scriptRef") + "</div><HR>";
        //$(tooltip).find("div.bg_popup-content").prepend(that.scriptRef);
      }
    } else {
      if (e.target.getAttribute("scriptRef") && e.target.getAttribute("scriptRef") != "" && $(tooltip).find("div[name=\"scriptRef\"]").length === 0) {
        that.scriptRef = "<div class=\"bg_popup-more\" name=\"scriptRef\">" + e.target.getAttribute("scriptRef") + "</div><HR>";
  ...
  var linkMouseout = function (e) {
    ...
    if (e.target.nodeName.toLowerCase() == "text" && showTimer) {
      window.clearTimeout(showTimer);
  ...
  that.updateTooltip = function (tooltip_content) {
    ...
    addCloseButton(tooltip);
    //new created div update in here
    if (that.scriptRef != "") {
      $(tooltip).find("div.bg_popup-content").prepend(that.scriptRef);
      that.scriptRef = "";
    }
    // plan to remove ref href a tag here
  };
